...
###reta e plano, funções revistas

def reta(a,b):
    """para uma equação da forma y=ax+b"""
    x = np.linspace(-10, 10) #intervalo de exibição
    y = a*x + b
...
def plano(a,b,c,d):
    ...
    x, y = np.meshgrid(x, y) #coordenadas
    z = (-a*x - b*y - d)/(c+1e-10)

    fig = plt.figure()
    ax = fig.add_subplot(111,projection='3d') #dimensiona e projeta
    ax.plot_surface(x,y,z, color='m', alpha=0.5) #plota o objeto com cor e transparencia
...
def epi_hipo(R, r, k):
    ...
    circumference = k * 360 #para completar os 360 graus
    x_points = []
    y_points = []
    ...
    rc = plt.Line2D((0, 0), (0, 0), linewidth=1, color="k") #marca o raio do circulo exterior
    circler = plt.Circle((0, 0), r, color='#e5047a', fill=False) #o círculo exterior (cuja trajetória marca a curva)
    circleR = plt.Circle((0, 0), R, color='#5b2eb7', fill=False) #o círculo interior
    punct = plt.Circle((0, 0), r / 10.0, color="k") #ponto que indica por onde o traço passa
    fig, ax = plt.subplots()
    ax.set_xlim(-5 * (R + r), 5 * (R + r))  #limites do plot
    ax.set_ylim(-5 * (R + r), 5 * (R + r))
    line, = ax.plot([], [], color='#166b28') #o traço da curva em si
    ax.add_artist(rc)
    ...
    def animate(i): #para animar a trajetoria
...
def helix(a, b):
    ...
    def update(frame): #para marcar os frames da animação
        t_frame = t[:frame]
        x = a*np.cos(t_frame)
        y = a*np.sin(t_frame)
        z = b*t_frame
